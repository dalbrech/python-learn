...
text = QtGui.QLineEdit("P[[1,2,10,6,2],[2,2,10,6,4]]; # Test pattern")
...
text.setToolTip("Enter manual timing here, in the format: \n"+
                "{pulse} P[[stim#,pulse_width(sec),pulse_interval(sec),#pulses,start_time(sec)]]; \n"+
                "{ramp}  R[[stim#,start_val(0-1),end_val(0-1),duration(sec),start_time(sec),steps]]; \n"+
                "{sine}  S[[stim#,start_val(0-1),end_val(0-1),duration(sec),period(sec),start_time(sec),steps]];")
...
def GenerateRamp(input):
    
# timing = GenerateRamp(input)
#
#   input is a n x 6 matrix, each row containing:
#
#       [valve,startlev,endlev,ramptime,first,steps]
#
#       valve    is an integer stimulus number
#       startlev is starting output level (0.0 - 1.0)
#       endlev   is ending output level (0.0 - 1.0)
#       ramptime is length of ramp (in seconds)
#       first    is ramp start (in seconds)
#       steps    is number of steps in ramp
#
#   example: input = [[1,0,1,10,5,10],[1,1,0,10,15,10]]
#               ramp from 0 -> 1 -> 0 from 5 -> 15 -> 25 s with 10 steps in channel 1.
#

    import numpy as np

    numpatterns = len(input)            # how many patterns (input rows)
    alltiming = np.empty((0,3), float)  # initialize timing array

    for p in range(0,numpatterns):      # loop through each row

        [valve,startlev,endlev,ramptime,first,steps] = input[p]  # populate vars for each row

        interval = ramptime / steps      # update interval
        ds = (endlev - startlev) / steps    # stimulus change per step
        timing = np.zeros((steps+1,3))
        timing[0] = [first, valve, startlev]
        for i in range(0, steps):
            timing[i+1] = [first + (i+0.5)*interval, valve, startlev + (i+1)*ds]
        alltiming = np.append(alltiming, timing, axis=0)    # add the next pattern to the list

    sortidx = np.argsort(alltiming, axis=0) # get sort index along columns
    alltiming = alltiming[sortidx[:,0]]     # rearrange according to first column (time)

    if (alltiming[0][0] > 0):    # set valve to startlev at time 0 if first is after t=0
        alltiming = np.insert(alltiming, 0, [0, alltiming[0][1], alltiming[0][2]], axis=0)
...
def GenerateSine(input):
    
# timing = GenerateSine(input)
#
#   input is a n x 7 matrix, each row containing:
#
#       [valve,startlev,endlev,ramptime,first,steps]
#
#       valve    is an integer stimulus number
#       startlev is starting output level (0.0 - 1.0)
#       endlev   is ending output level (0.0 - 1.0)
#       duration is length of sinusoid (in seconds)
#       period   is length of one cycle (in seconds)
#       first    is sine start (in seconds)
#       steps    is number of steps in sine period
#
#   example: input = [[1,0,1,10,5,5,50]]
#   
#
    ...
    for p in range(0,numpatterns):      # loop through each row
        ...
        [valve,startlev,endlev,duration,period,first,steps] = input[p]  # populate vars for each row

        interval = period / steps       # update interval
        numsteps = int(np.ceil(duration / interval))     # total # of steps
        amp = endlev-startlev           # amplitude of sine wave
        
        timing = np.zeros((numsteps+1,3))
        timing[0] = [first, valve, startlev]
        for i in range(0, numsteps):
            timing[i+1] = [first + (i+0.5)*interval, valve, \
                           startlev + amp/2*(1-np.cos((i+1)*interval*2*np.pi/period))]
        alltiming = np.append(alltiming, timing, axis=0)    # add the next pattern to the list
...
def clickEnter():
    import ast
    global time
    textentry = text.text().replace(' ','')
    print(textentry)
    if (textentry[0] == '+'):        # use + as first character to append
        print("Adding stimulus patterns")
        textentry = textentry[1:]
    else:
        time = np.empty((0,3), int)  # otherwise clear time list
        
    sections = textentry.split(';')  # split text by ;-delimited sections
    for current in sections:
        try:
            stimtype = current[0]    # character defining type of stimulus:
                                     #    P = pulse; R = ramp; S = sine;
            current = current[1:]
            print(stimtype)
            pattern = ast.literal_eval(current)
            print(pattern)
            if (stimtype == 'P'):
                #time = GenerateMultiPulsePattern(pattern)
                time = np.append(time, GenerateMultiPulsePattern(pattern), axis=0)
            elif (stimtype == 'R'):
                time = np.append(time, GenerateRamp(pattern), axis=0)
            elif (stimtype == 'S'):
                time = np.append(time, GenerateSine(pattern), axis=0)
            sortidx = np.argsort(time, axis=0) # get sort index along columns
            time = time[sortidx[:,0]]     # rearrange according to first column (time)
