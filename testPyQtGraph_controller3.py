...
start.setStyleSheet('QPushButton {background-color: #80FF80; font-size: 18px}')
stop.setStyleSheet('QPushButton {background-color: #FF8080; font-size: 18px}')
...
pos = 0
vert = []
refresh = 0.1 # time marker refresh interval
...
def updateDisplay():
    ...
    for p in pinsUsed:
        ...
        y = np.squeeze(time[ii,1]+time[ii,2]*0.8 - 0.4)
        #graph.plot(np.append(x,x[-1]), y, pen=(p,8), stepMode=True)
        graph.plot(np.append(x,maxtime), y, pen=(p,8), stepMode=True)
    global vert
